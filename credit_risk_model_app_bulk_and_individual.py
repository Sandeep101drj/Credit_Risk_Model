...
page = st.sidebar.radio("Go to", ["💳 Credit Risk Prediction", "📊 CSV Prediction Results", "ℹ️ About"])
...
if page == "💳 Credit Risk Prediction":
    ...
    with st.container():
        ...
        with col4:
            loan_percent_income = st.number_input('📉 Percentage of Income spent on loan (%)', min_value=0.0, max_value=1.0, step=0.1, help="Enter the percentage of income spent on loan.")
...
elif page == "📊 CSV Prediction Results":
    ...
    |-------------------------------------|----------------|-----------------------------------------------|
    | `person_age`                        | Numeric (int)  | Age of the borrower (18-100)                  |
    ...
    | `loan_int_rate`                     | Numeric (float)| Loan interest rate (1-30%)                   |
    | `loan_percent_income`               | Numeric (float)| Percentage of income spent on loan           |
    ...
    if uploaded_file is not None:
        ...
        if all(col in input_data.columns for col in required_columns):
            ...
            transformed_input = transformer.transform(input_data[required_columns])
